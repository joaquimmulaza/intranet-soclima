...
<div class="containerBtnFiltrar">
    ...
    <div class="tooltipFiltros">
        <ul class="filter-options">
            <li class="filter-option" data-status="Todos">Todos</li>
            <li class="filter-option" data-status="Pendente">Pendente</li>
            <li class="filter-option" data-status="Aprovado">Aprovado</li>
            <li class="filter-option" data-status="Rejeitado">Rejeitado</li>
        </ul>
    </div>
</div>
...
<a class="view_justificativos" href="{{ route('documents.visualizar', $ausencia->id) }}">
    ...
    <table class="docs_table view_justificativos" data-status="{{ trim($ausencia->status) }}">
        <thead>
            <tr>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const filterButton = document.querySelector('.btnFiltrar');
    const filterText = filterButton.querySelector('.filter-text');
    const filterOptions = document.querySelectorAll('.filter-option');
    const tables = document.querySelectorAll('.docs_table');
    const tooltip = document.querySelector('.tooltipFiltros');

    // Depuração: Logar os valores de data-status
    tables.forEach(table => {
        console.log('Table data-status:', table.dataset.status);
    });

    // Atualiza o texto do botão e filtra as tabelas
    filterOptions.forEach(option => {
        option.addEventListener('click', () => {
            const selectedStatus = option.dataset.status;
            filterText.textContent = selectedStatus; // Atualiza apenas o texto

            // Depuração: Logar o status selecionado
            console.log('Selected status:', selectedStatus);

            // Filtra as tabelas
            tables.forEach(table => {
                const tableStatus = table.dataset.status || '';
                const parent = table.parentElement; // O <a> que envolve a tabela

                // Depuração: Logar a comparação
                console.log('Comparing:', tableStatus, 'with', selectedStatus);

                if (selectedStatus === 'Todos' || tableStatus === selectedStatus) {
    ...
    document.addEventListener('click', (event) => {
        if (!event.target.closest('.containerBtnFiltrar')) {
            tooltip.style.display = 'none';
        }
    });

    // Impede redirecionamento ao clicar nos botões de ação
    document.querySelectorAll('.view_justificativos').forEach(link => {
        link.addEventListener('click', (event) => {
            if (event.target.closest('.containerOpt, .btn-popup, .modal')) {
                event.preventDefault();
            }
        });
    });
